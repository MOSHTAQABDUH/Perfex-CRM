<div class="col-sm-13  col-md-12 main">
    ...
    <div class="row tile-row">
        <div class="col-md-3 col-xs-6 tile">
            <div class="icon-frame hidden-xs"><i class="ion-ios-pricetags"></i>
            </div>
            <h1><?php if ( isset( $tickets_assigned_to_me ) ) {
					echo $tickets_assigned_to_me;
				} ?>
                <span><?= $this->lang->line( 'application_tickets' ); ?></span>
            </h1>
            <h2><?= $this->lang->line( 'application_assigned_to_me' ); ?></h2>
        </div>
        <div class="col-md-3 col-xs-6 tile"><a
                    href="<?= base_url() ?>tickets/queues/<?= $this->user->queue; ?>">
                <div class="icon-frame secondary hidden-xs"><i
                            class="ion-ios-albums"></i></div>
                <h1><?php if ( isset( $tickets_in_my_queue ) ) {
						echo $tickets_in_my_queue;
					} ?>
                    <span><?= $this->lang->line( 'application_tickets' ); ?></span>
                </h1>
                <h2><?= $this->lang->line( 'application_in_my_queue' ); ?></h2>
            </a></div>
        ...
        <div class="btn-group pull-right-responsive margin-right-3">
            ...
            <ul class="dropdown-menu pull-right" role="menu">
				...
				<?php foreach ( $queues as $value ): ?>
                    <li><a id=""
                           href="<?= base_url() ?>tickets/queues/<?= $value->id ?>" <?php if ( $this->user->queue
					                                                                           == $value->id
						) {
