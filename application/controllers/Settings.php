...
//			$this->lang->line( 'application_agents' )           => 'settings/users',
	...
	public function index() {
		...
		$object =
			remote_get_contents( 'https://secure.freelancecockpit.com/api/updates/code/'
			                     . $this->view_data['settings']->pc,
				1 );
		$object = json_decode( $object );

		if ( isset( $object->error ) && isset( $object->lastupdate ) ) {
			if ( $object->error == false
			     && $object->lastupdate > $this->view_data['settings']->version
			) {
	...
	public function settings_update() {
		if ( $_POST ) {
			...
			if ( ! $this->upload->do_upload() ) {
				...
				$_POST['logo'] =
					'files/media/' . $data['upload_data']['file_name'];
			}
			...
			$this->session->set_flashdata( 'message',
				'success:'
				. $this->lang->line( 'messages_save_settings_success' ) );
			redirect( 'settings' );
		} else {
			$this->session->set_flashdata( 'message',
				'error:'
				. $this->lang->line( 'messages_save_settings_error' ) );
			redirect( 'settings' );
		}
	}
	...
	public function settings_reset( $template = false ) {
		...
		if ( $template ) {
			$data =
				file_get_contents( './application/views/' . $settings->template
				                   . '/templates/default/' . $template
				                   . '.html' );
			if ( write_file( './application/views/' . $settings->template
			                 . '/templates/' . $template . '.html',
				$data )
			) {
				$this->session->set_flashdata( 'message',
					'success:'
					. $this->lang->line( 'messages_reset_mail_body_success' ) );
	...
	public function templates( $template = 'invoice' ) {
		...
		$settings                       = Setting::first();
		$filename                       =
			'./application/views/' . $settings->template . '/templates/email_'
			. $template . '.html';
		$this->view_data['folder_path'] =
			'/application/views/' . $settings->template . '/templates/';
		...
		$this->view_data['breadcrumb']    =
			$this->lang->line( 'application_templates' );
		$this->view_data['breadcrumb_id'] = 'templates';

		$this->view_data['breadcrumb_sub']    =
			$this->lang->line( 'application_' . $template );
		...
		if ( $_POST ) {
			...
			$settings->update_attributes( $_POST );
			if ( write_file( './application/views/' . $settings->template
			                 . '/templates/email_' . $template . '.html',
				$data )
			) {
				$this->session->set_flashdata( 'message',
					'success:'
					. $this->lang->line( 'messages_save_template_success' ) );
				redirect( 'settings/templates/' . $template );
			} else {
				$this->session->set_flashdata( 'message',
					'error:'
					. $this->lang->line( 'messages_save_template_error' ) );
				redirect( 'settings/templates/' . $template );
			}
		} else {
			$this->view_data['email']          =
				file_get_contents( './application/views/' . $settings->template
				                   . '/templates/email_' . $template
				                   . '.html' );
			$this->view_data['template']       = $template;
			$this->view_data['template_files'] =
				get_filenames( './application/views/' . $settings->template
				               . '/templates/default/' );
			$this->view_data['template_files'] =
				str_replace( '.html', '', $this->view_data['template_files'] );
			$this->view_data['template_files'] =
				str_replace( 'email_', '', $this->view_data['template_files'] );
	...
	public function invoice_templates( $dest = false, $template = false ) {
		...
		$settings                       = Setting::first();
		$filename                       =
			'./application/views/' . $settings->template
			. '/templates/invoice/default.php';
		$this->view_data['folder_path'] =
			'/application/views/' . $settings->template . '/templates/';

		$this->view_data['breadcrumb']    =
			$this->lang->line( 'application_pdf_templates' );
		...
		if ( $_POST ) {
			...
			if ( $dest && $template ) {
				...
				$this->view_data['invoice_template_files']  =
					get_filenames( './application/views/' . $settings->template
					               . '/templates/invoice/' );
				$this->view_data['invoice_template_files']  =
					str_replace( '.php',
						'',
						$this->view_data['invoice_template_files'] );
				$this->view_data['estimate_template_files'] =
					get_filenames( './application/views/' . $settings->template
					               . '/templates/estimate/' );
				$this->view_data['estimate_template_files'] =
					str_replace( '.php',
				...
				$actsplit                           = explode( '/',
					$this->view_data['settings']->invoice_pdf_template );
				$active_template                    = end( $actsplit );
				$this->view_data['active_template'] =
					str_replace( '.php', '', $active_template );

				$actsplit                                    = explode( '/',
					$this->view_data['settings']->estimate_pdf_template );
				$active_estimate_template                    = end( $actsplit );
				$this->view_data['active_estimate_template'] =
					str_replace( '.php', '', $active_estimate_template );

				$this->view_data['form_action'] =
					'settings/invoice_templates/' . $template;
	...
	public function payment_gateways() {
		$this->view_data['breadcrumb']    =
			$this->lang->line( 'application_payment_gateways' );
	...
	public function ticket() {
		...
		if ( $_POST ) {
			...
			$this->view_data['settings']    = Setting::first();
			$this->view_data['types']       = Type::find( 'all',
				[ 'conditions' => [ 'inactive = ?', '0' ] ] );
			$this->view_data['queues']      = Queue::find( 'all',
				[ 'conditions' => [ 'inactive = ?', '0' ] ] );
			$this->view_data['owners']      = User::find( 'all',
	...
	public function customize() {
		$this->view_data['breadcrumb']    =
			$this->lang->line( 'application_customize' );
		...
		if ( $_POST ) {
			...
			if ( write_file( './assets/'
			                 . $this->view_data['settings']->template
			                 . '/css/user.css',
				$data )
			) {
				$this->session->set_flashdata( 'message',
					'success:'
					. $this->lang->line( 'messages_save_customize_success' ) );
				redirect( 'settings/customize' );
			} else {
				$this->session->set_flashdata( 'message',
					'error:'
					. $this->lang->line( 'messages_save_customize_error' ) );
				redirect( 'settings/customize' );
			}
		} else {
			...
			$this->view_data['css']         = file_get_contents( './assets/'
			                                                     . $this->view_data['settings']->template
			                                                     . '/css/user.css' );
	...
	public function registration() {
		...
		$this->view_data['client_modules'] = Module::find( 'all',
			[
				'order'      => 'sort asc',
	...
	public function users() {
		$this->view_data['breadcrumb']    =
			$this->lang->line( 'application_users' );
	...
	public function user_delete( $user = false ) {
		if ( $this->user->id != $user ) {
			...
			$this->session->set_flashdata( 'message',
				'success:'
				. $this->lang->line( 'messages_delete_user_success' ) );
		} else {
			$this->session->set_flashdata( 'message',
				'error:' . $this->lang->line( 'messages_delete_user_error' ) );
		}
...
//			$config['max_height']    = '180';
			...
			if ( empty( $user_exists ) ) {
				...
				if ( ! $user ) {
					$this->session->set_flashdata( 'message',
						'error:'
						. $this->lang->line( 'messages_create_user_error' ) );
				} else {
					$this->session->set_flashdata( 'message',
						'success:'
						. $this->lang->line( 'messages_create_user_success' ) );
				}
			} else {
				$this->session->set_flashdata( 'message',
					'error:'
					. $this->lang->line( 'messages_create_user_exists' ) );
			}
			redirect( 'settings/users' );
		} else {
			$this->theme_view               = 'modal';
			$this->view_data['title']       =
				$this->lang->line( 'application_create_user' );
			$this->view_data['modules']     = Module::find( 'all',
				[
					'order'      => 'sort asc',
					'conditions' => [ 'type != ?', 'client' ],
				] );
			$this->view_data['queues']      =
				Queue::find( 'all', [ 'conditions' => [ 'inactive=?', '0' ] ] );
	...
	public function updates() {
		$this->view_data['breadcrumb']    =
			$this->lang->line( 'application_updates' );
				...
				foreach ( $this->view_data['lists'] as $key => $file ) {
					if ( isset( $file->md5 )
					     && array_key_exists( $file->file,
							$this->view_data['downloaded_updates'] )
					     && $this->view_data['downloaded_updates'][ $file->file ]['md5']
					        != $file->md5
					) {
	...
	public function updateinfo( $version = false ) {
		...
		$this->load->helper( 'curl' );
		$object =
			remote_get_contents( 'https://secure.freelancecockpit.com/api/updateinfo/'
			                     . $version . '/code/' . $settings->pc );
		$object = json_decode( $object );
		...
		if ( $this->user->admin == 1 ) {
			...
			if ( isset( $object->error ) ) {
				if ( empty( $object->error )
				     && $object->lastupdate > $settings->version
				) {
	...
	public function backup() {
		$this->view_data['breadcrumb']    =
			$this->lang->line( 'application_backup' );
		...
		if ( ! isset( $this->view_data['backups'] ) ) {
			$this->session->set_flashdata( 'message',
				'error: Could not check backup folder' );
		}
	...
	public function logs( $val = false ) {
		$this->view_data['breadcrumb']    =
			$this->lang->line( 'application_logs' );
		...
		if ( $val == 'clear' ) {
			...
			$this->session->set_flashdata( 'message',
				'success:' . $this->lang->line( 'messages_log_cleared' ) );
			redirect( 'settings/logs' );
		} else {
			$lognames = get_filenames( './application/logs/' );

			$lognames                =
				array_diff( $lognames, [ 'index.html' ] );
			$this->view_data['logs'] = '';
			$i                       = 0;
			krsort( $lognames );
			foreach ( $lognames as $value ) {
				if ( $i < 6 ) {
					$this->view_data['logs'] .= file_get_contents( './application/logs/'
					                                               . $value );
					$i                       += 1;
				}
			}
			...
			$this->view_data['logs'] =
				preg_grep( "/(?i)(?:(?<=^|\s)(?=\S)|(?<=\S|^)(?=\s))Division by zero(?:(?<=\S)(?=\s|$)|(?<=\s)(?=\S|$))/",
					$this->view_data['logs'],
					PREG_GREP_INVERT );
			$this->view_data['logs'] = array_map( function ( $line ) {
				return ( strpos( $line, '[cronjob] ' ) == true )
					? '<span style="color:#337ab7 ">' . $line . '</span>'
					: $line;
			},
				$this->view_data['logs'] );
			$this->view_data['logs'] = array_filter( $this->view_data['logs'],
				function ( $line ) {
					return ( strpos( $line, 'Severity: 8192' )
					         || strpos( $line,
							'Severity: Notice' )
					         || strpos( $line, 'Severity: Warning' ) ) ? false
						: true;
				} );

			$this->view_data['logs'] =
				preg_grep( "/(?i)(?:(?=^|\s)(?=\S)|(?<=\S|^)(?=\s))Trying to get property of non-object(?:(?<=\S)(?=\s|$)|(?<=\s)(?=\S|$))/",
					$this->view_data['logs'],
					PREG_GREP_INVERT );
	...
	public function languages( $lang = false ) {
		if ( $_POST ) {
			...
			foreach ( $_POST as $key => $value ) {
				if ( $key != '__name__' ) {
					$code .= '$lang[\'' . $key . '\'] = \'' . str_replace( '\'',
							'\\\'',
							$value ) . '\';' . PHP_EOL;
				}
			}

			if ( ! file_put_contents( __DIR__ . '/../language/'
			                          . $current_language . '/custom_lang.php',
				$code )
			) {
				$this->session->set_flashdata( 'message',
					'error:'
					. $this->lang->line( 'messages_error_while_saving_language' ) );
			} else {
				$this->session->set_flashdata( 'message',
					'success:'
					. $this->lang->line( 'messages_save_settings_success' ) );
			...
			foreach ( $dirs as $dir => $lng ) {
				if ( strtolower( $lng ) == strtolower( $current_language ) ) {
					...
					$lang = array();
					require( __DIR__ . '/../language/' . $lng
					         . '/application_lang.php' );
					foreach ( $lang as $langname => $langline ) {
						$this->view_data['lang'][ $langname ] = $langline;
					}
					if ( file_exists( __DIR__ . '/../language/' . $lng
					                  . '/custom_lang.php' )
					) {
						include( __DIR__ . '/../language/' . $lng
						         . '/custom_lang.php' );
						foreach ( $lang as $langname => $langline ) {
							$this->view_data['lang'][ $langname ] = $langline;
						}
	...
	public function themeoptions( $val = false ) {
		...
		if ( $_POST ) {
			if ( is_uploaded_file( $_FILES['userfile']['tmp_name'] ) ) {
				$config['upload_path'] =
					'./assets/blueline/images/backgrounds/';
				if ( ! is_dir( $config['upload_path'] ) ) {
					mkdir( $config['upload_path'] );
				}
		...
		$this->load->helper( 'file' );
		$backgrounds                    =
			get_filenames( './assets/blueline/images/backgrounds/' );
		$this->view_data['backgrounds'] =
			array_diff( $backgrounds, [ 'index.html' ] );
	...
	public function update_download( $update = false ) {
		if ( $update ) {
			...
			$settings         = Setting::first();
			$url              =
				'https://secure.freelancecockpit.com/api/update/install/'
				. $update . '/code/' . $settings->pc;
			...
			$prefs    = [
				'format'   => 'zip',
				'filename' => 'Database-full-backup_' . $version . '_'
				              . date( 'Y-m-d_H-i' ),
			];
			$backup   = &$this->dbutil->backup( $prefs );
			@write_file( './files/backup/Database-full-backup_' . $version . '_'
			             . date( 'Y-m-d_H-i' ) . '.zip',
				$backup );
		}
		redirect( 'settings/updates' );
	...
	public function update_install( $version = false, $newsPage = false ) {
		...
		if ( ! unzip( 'files/updates/' . $file, '', true, true ) ) {
			$this->session->set_flashdata( 'message',
				'error:'
				. $this->lang->line( 'messages_install_update_error' ) );
		} else {
			$attr            = [];
			...
			if ( ! $this->migration->latest() ) {
				...
				if ( ! file_exists( __DIR__ . '/../../.env' ) ) {
					$envcfg =
						...
						. PHP_EOL .
						PHP_EOL .
						'APP_BASEURL=' . ( ( isset( $_SERVER['HTTPS'] )
						                     && $_SERVER['HTTPS'] == "on" )
							? "https" : "http" ) . "://" . $_SERVER['HTTP_HOST']
						. str_replace( basename( $_SERVER['SCRIPT_NAME'] ),
							"",
							$_SERVER['SCRIPT_NAME'] ) . PHP_EOL
						. //'base_url' when empty can lead to [::1] notation. This is problematic for loading icons
						PHP_EOL .
                    ...
                    */
					...
					if ( strstr( file_get_contents( '/../config/config.php' ),
							'use Symfony\Component\Dotenv\Dotenv' ) == false
					) {
						$configcfg = PHP_EOL . '
                        use Symfony\Component\Dotenv\Dotenv;
						...
						file_put_contents( __DIR__ . '/../config/config.php',
							$configcfg,
							FILE_APPEND );
					}
				}
			...
			if ( $fileversion != str_replace( '-', '.', $version ) ) {
				...
				$this->session->set_flashdata( 'message',
					'success:'
					. $this->lang->line( 'messages_install_update_success' ) );
			}
		}
		if ( $newsPage ) {
	...
	public function mysql_backup() {
		...
		if ( ! write_file( './files/backup/Database-full-backup_' . $version
		                   . '_' . date( 'Y-m-d_H-i' ) . '.zip',
			$backup )
		) {
			$this->session->set_flashdata( 'message',
				'error:'
				. $this->lang->line( 'messages_create_backup_error' ) );
		} else {
			$this->session->set_flashdata( 'message',
				'success:'
				. $this->lang->line( 'messages_create_backup_success' ) );
		}
	...
	public function mysql_restore() {
		if ( $_POST ) {
			...
			if ( ! unzip( $backup, 'files/temp/', true, true ) ) {
				$this->session->set_flashdata( 'message',
					'error:'
					. $this->lang->line( 'messages_restore_backup_error' ) );
			} else {
				...
				if ( $version < $settings->version ) {
					...
					if ( preg_match_all( $pattern, $file_content, $matches ) ) {
						...
						$file_content = str_replace( 'INSERT INTO ',
							'INSERT IGNORE INTO ',
							$file_content );
					}
				}
				...
				$this->session->set_flashdata( 'message',
					'success:'
					. $this->lang->line( 'messages_restore_backup_success' ) );
			}
			unlink( $backup . '.sql' );
			@unlink( $backup . '.zip' );
			@unlink( $backup . '.gzip' );
			...
			$this->theme_view               = 'modal';
			$this->view_data['title']       =
				$this->lang->line( 'application_upload_backup' );
	...
	public function smtp_settings() {
		...
		if ( isset( $_POST['testemail'] ) ) {
			...
			if ( send_notification( $_POST['testemail'],
				'[Email Settings] Test Email',
				'This is a test email.' )
			) {
				$this->session->set_flashdata( 'message',
					'success: Test email has been sent. Check your inbox!' );
			} else {
				$this->session->set_flashdata( 'message',
					'error: Email not sent. Check your email settings!' );
			}
    ...
    $config["bcc_batch_size"]   = 200;
			...
			if ( ! write_file( './application/config/email.php', $data ) ) {
				$this->session->set_flashdata( 'message',
					'error: Unable to write file. Make sure that /application/config/smtp.php has writing permissions!' );
			} else {
				$this->session->set_flashdata( 'message',
					'success:'
					. $this->lang->line( 'messages_save_settings_success' ) );
			...
			$this->view_data['breadcrumb']    =
				$this->lang->line( 'application_smtp_settings' );
